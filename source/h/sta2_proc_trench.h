...
int Count_Tiles_Trench(int x, int y);
int Proc_Get_Pixel_Trench(int pixel, int row_x, int row_y);
void sta2_Proc_Level_Trench(); 
 
#define TEST_ROUTE_MAP  0

#define TR_ROOM_EMPTY        0
#define TR_ROOM_ROAD_RIGHT   1
#define TR_ROOM_ROAD_UP      2
#define TR_ROOM_ROAD_RIGHT1  3
#define TR_ROOM_ROAD_RIGHT2  4
#define TR_ROOM_ROAD_CROSS   5
#define TR_ROOM_ROAD_CROSSU   6
#define TR_ROOM_ROAD_CROSSD   7
#define TR_ROOM_ROAD_CROSSL   8
#define TR_ROOM_ROAD_CROSSR   9

#define TR_ROOM_TREE         10
#define TR_ROOM_ROCK         11

#define TR_ROOM_HUT1     12
#define TR_ROOM_HUT2     13
#define TR_ROOM_HUT3     14
#define TR_ROOM_HUT4     15
#define TR_ROOM_HUT5     16
#define TR_ROOM_HUT6     17


#define TR_ROOM_FULL        20
#define TR_ROOM_BUSH_UP     21
#define TR_ROOM_BUSH_DOWN   22
#define TR_ROOM_BUSH_LEFT   23
#define TR_ROOM_BUSH_RIGHT  24
#define TR_ROOM_BUSH_UPLEFT    25
#define TR_ROOM_BUSH_UPRIGHT   26
#define TR_ROOM_BUSH_DOWNLEFT   27
#define TR_ROOM_BUSH_DOWNRIGHT  28
#define TR_ROOM_BUSH_LEFTRIGHT  29
#define TR_ROOM_BUSH_ENDLEFT    30
#define TR_ROOM_BUSH_ENDRIGHT   31
#define TR_ROOM_BUSH_ENDUP      32
#define TR_ROOM_BUSH_ENDDOWN    33
#define TR_ROOM_BUSH_UPDOWN     34
...
void sta2_Proc_Level_Trench()   
{
	...
	{      
		...
		if(rand()%4 == 0 && sta2_is_first_level == false && sta2_final_boss == false) 
	...
	for(int i = 0;i < index;i++)  
	{
		...
		{ 
			...
			if(gotoX >= 0 && gotoX < (MAP_WIDTH/2) && gotoY >= 0 && gotoY < (MAP_HEIGHT/2))
				room[gotoY][gotoX] = TR_ROOM_FULL;
			if(gotoX >= 0 && gotoX < (MAP_WIDTH/2) && (gotoY-1) >= 0 && (gotoY-1) < (MAP_HEIGHT/2))
				room[gotoY-1][gotoX] = TR_ROOM_FULL;
...
//	if(rand()%3 != 0)
	for(int i = 0;i < 1;i++)    
	{
		...
		{ 
			...
			if(gotoX >= 0 && gotoX < (MAP_WIDTH/2) && gotoY >= 0 && gotoY < (MAP_HEIGHT/2))
				room[gotoY][gotoX] = TR_ROOM_ROAD_RIGHT; 
...
//	if(rand()%3 != 0)
	...
	/*	#define TR_ROOM_ROAD_RIGHT   1 
		...
		// Bush edges...
		if(room[y][x] >= TR_ROOM_ROAD_RIGHT && room[y][x] <= TR_ROOM_ROAD_CROSSR)
		{

			gotoX = x;	gotoY = y+1;
			if(gotoX >= 0 && gotoY >= 0 && gotoX < (MAP_WIDTH/2) && gotoY < (MAP_HEIGHT/2))
			if(room[gotoY][gotoX] >= TR_ROOM_ROAD_RIGHT && room[gotoY][gotoX] <= TR_ROOM_ROAD_CROSSR)
				tilecol[UP] = 1; 
			if(gotoX < 0 || gotoY < 0 || gotoX >= (MAP_WIDTH/2) || gotoY >= (MAP_HEIGHT/2))
				tilecol[UP] = 1;  
	...
	for(int i = 0;i < 35;i++)     
	{
		...
		if(room[y][x] == TR_ROOM_EMPTY) 
		{
			...
			for(int j=-2;j<3;j++) 
			{
				...
				if(gotoX >= 0 && gotoX < (MAP_WIDTH/2) && gotoY >= 0 && gotoY < (MAP_HEIGHT/2))
				if(room[gotoY][gotoX] != TR_ROOM_EMPTY)
					continue_on = false;
			}

			if(continue_on == true)
				room[y][x] = TR_ROOM_TREE;
		}
	...
	{     
		...
		{ 
			...
			{  
				...
				if(room[temp_row_y][temp_row_x] == TR_ROOM_FULL || room[temp_row_y][temp_row_x] == TR_ROOM_TREE || room[temp_row_y][temp_row_x] == TR_ROOM_ROCK)
					room[temp_row_y][temp_row_x] = TR_ROOM_EMPTY;
	...
	{ 
		...
		{
			if(room[y][x] == TR_ROOM_FULL)
			if(Count_Tiles_Trench(x, y) == 0)
				room[y][x] = TR_ROOM_EMPTY; 

			if(room[y][x] == TR_ROOM_EMPTY)  
			if(Count_Tiles_Trench(x, y) >= 4) 
				room[y][x] = TR_ROOM_FULL;
		}
	}
	...
	{ 
		...
		{ 
			...
			if(room[y][x] != TR_ROOM_EMPTY || room_col[y][x]==true)  
				continue_on = false;
			if(room[y][x+1] != TR_ROOM_EMPTY || room_col[y][x+1]==true)  
				continue_on = false;
			 
			if(room[y+1][x] != TR_ROOM_EMPTY || room_col[y+1][x]==true) 
				continue_on = false;
			if(room[y+1][x+1] != TR_ROOM_EMPTY || room_col[y+1][x+1]==true)  
				continue_on = false;

			if(rand()%12 == 0)  
			if(continue_on == true)
			{
				room[y+1][x] = TR_ROOM_HUT1;
				room[y+1][x+1] = TR_ROOM_HUT2;
				room[y][x] = TR_ROOM_HUT3;
				room[y][x+1] = TR_ROOM_HUT4;
	...
	{ 
		...
		if(room[y][x] == TR_ROOM_FULL) 
		{
			...
			gotoX = x;	gotoY = y;
			if(gotoX >= 0 && gotoY >= 0 && gotoX < (MAP_WIDTH/2) && gotoY < (MAP_HEIGHT/2))
			if(room[gotoY][gotoX] >= TR_ROOM_FULL && room[gotoY][gotoX] <= TR_ROOM_BUSH_UPDOWN)
					tilecol[RESET] = 1;  
			if(gotoX < 0 || gotoY < 0 || gotoX >= (MAP_WIDTH/2) || gotoY >= (MAP_HEIGHT/2))
				tilecol[RESET] = 1;  
	...
	{  
		...
		if(room[y][x] == TR_ROOM_ROAD_RIGHT)     
			tile_textureID = 96;
		if(room[y][x] == TR_ROOM_ROAD_UP)   
			tile_textureID = 98; 
		if(room[y][x] == TR_ROOM_ROAD_RIGHT1)
			tile_textureID = 100;    
		if(room[y][x] == TR_ROOM_ROAD_RIGHT2)
			tile_textureID = 102;
		if(room[y][x] == TR_ROOM_ROAD_CROSS) 
			tile_textureID = 104;
		if(room[y][x] == TR_ROOM_ROAD_CROSSU) 
			tile_textureID = 106;
		if(room[y][x] == TR_ROOM_ROAD_CROSSD) 
			tile_textureID = 108;
		if(room[y][x] == TR_ROOM_ROAD_CROSSL) 
			tile_textureID = 110;
		if(room[y][x] == TR_ROOM_ROAD_CROSSR) 
			tile_textureID = 128;

		if(room[y][x] == TR_ROOM_HUT1) 
			tile_textureID = 130;
		if(room[y][x] == TR_ROOM_HUT2)  
			tile_textureID = 132;
		if(room[y][x] == TR_ROOM_HUT3) 
			tile_textureID = 134;
		if(room[y][x] == TR_ROOM_HUT4) 
			tile_textureID = 136;
		if(room[y][x] == TR_ROOM_HUT5) 
			tile_textureID = 138;
		if(room[y][x] == TR_ROOM_HUT6)  
			tile_textureID = 140;

		if(room[y][x] == TR_ROOM_ROCK) 
			tile_textureID = 142;
		
		if(room[y][x] == TR_ROOM_FULL)
			tile_textureID = 32;
		if(room[y][x] == TR_ROOM_BUSH_UP)    
			tile_textureID = 34;
		if(room[y][x] == TR_ROOM_BUSH_DOWN)      
			tile_textureID = 36;
		if(room[y][x] == TR_ROOM_BUSH_LEFT) 
			tile_textureID = 38;
		if(room[y][x] == TR_ROOM_BUSH_RIGHT)
			tile_textureID = 40;
		if(room[y][x] == TR_ROOM_BUSH_UPLEFT)   
			tile_textureID = 42;
		if(room[y][x] == TR_ROOM_BUSH_UPRIGHT)
			tile_textureID = 44;
		if(room[y][x] == TR_ROOM_BUSH_DOWNLEFT)
			tile_textureID = 46;
		if(room[y][x] == TR_ROOM_BUSH_DOWNRIGHT) 
			tile_textureID = 64;
		if(room[y][x] == TR_ROOM_BUSH_LEFTRIGHT)
			tile_textureID = 66;
		if(room[y][x] == TR_ROOM_BUSH_ENDLEFT)
			tile_textureID = 68;
		if(room[y][x] == TR_ROOM_BUSH_ENDRIGHT)
			tile_textureID = 70;
		if(room[y][x] == TR_ROOM_BUSH_ENDUP)
			tile_textureID = 72;
		if(room[y][x] == TR_ROOM_BUSH_ENDDOWN)
			tile_textureID = 74;
		if(room[y][x] == TR_ROOM_BUSH_UPDOWN)
			tile_textureID = 76;
		if(room[y][x] == TR_ROOM_TREE)
			tile_textureID = 78; 
		...
		if(tileIndex >= 0 && tileIndex < MAP_TILES) 
		{
			...
			if(tileIndex >= 0 && tileIndex < MAP_TILES)
			{
				...
				if(room[y][x] >= TR_ROOM_FULL || room[y][x] == TR_ROOM_TREE)
				if(room[y][x] != TR_ROOM_BUSH_DOWNLEFT) 
				if(room[y][x] != TR_ROOM_BUSH_ENDDOWN) 
				if(room[y][x] != TR_ROOM_TREE)
				{
			...
			if( (tileIndex+MAP_WIDTH) >= 0 && (tileIndex+MAP_WIDTH) < MAP_TILES)
			{
				...
				if(room[y][x] >= TR_ROOM_FULL || room[y][x] == TR_ROOM_TREE || room[y][x] == TR_ROOM_ROCK)
				if(room[y][x] != TR_ROOM_BUSH_UPLEFT) 
				if(room[y][x] != TR_ROOM_BUSH_ENDUP)        
				{    
...
{ 
	...
	{ 
		...
		if(room[my_row_y][my_row_x] >= TR_ROOM_FULL)        
		{           
			frameX = 0;
			frameY = 1; 
			 
			if(room[my_row_y][my_row_x] == TR_ROOM_FULL){frameX = 0;frameY = 1;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_UP){frameX = 1;frameY = 1;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_DOWN){frameX = 2;frameY = 1;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_LEFT){frameX = 3;frameY = 1;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_RIGHT){frameX = 4;frameY = 1;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_UPLEFT){frameX = 5;frameY = 1;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_UPRIGHT){frameX = 6;frameY = 1;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_DOWNLEFT){frameX = 7;frameY = 1;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_DOWNRIGHT){frameX = 0;frameY = 2;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_LEFTRIGHT){frameX = 1;frameY = 2;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_ENDLEFT){frameX = 2;frameY = 2;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_ENDRIGHT){frameX = 3;frameY = 2;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_ENDUP){frameX = 4;frameY = 2;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_ENDDOWN){frameX = 5;frameY = 2;}
			if(room[my_row_y][my_row_x] == TR_ROOM_BUSH_UPDOWN){frameX = 6;frameY = 2;}
		...
		if(room[my_row_y][my_row_x] >= TR_ROOM_ROAD_RIGHT && room[my_row_y][my_row_x] <= TR_ROOM_ROAD_CROSSR)
		{
			if(room[my_row_y][my_row_x] == TR_ROOM_ROAD_RIGHT){frameX = 0;frameY = 3;}
			if(room[my_row_y][my_row_x] == TR_ROOM_ROAD_UP){frameX = 1;frameY = 3;}
			if(room[my_row_y][my_row_x] == TR_ROOM_ROAD_RIGHT1){frameX = 2;frameY = 3;}
			if(room[my_row_y][my_row_x] == TR_ROOM_ROAD_RIGHT2){frameX = 3;frameY = 3;} 
			if(room[my_row_y][my_row_x] == TR_ROOM_ROAD_CROSS){frameX = 4;frameY = 3;}
			if(room[my_row_y][my_row_x] == TR_ROOM_ROAD_CROSSU){frameX = 5;frameY = 3;}
			if(room[my_row_y][my_row_x] == TR_ROOM_ROAD_CROSSD){frameX = 6;frameY = 3;}
			if(room[my_row_y][my_row_x] == TR_ROOM_ROAD_CROSSL){frameX = 7;frameY = 3;}
			if(room[my_row_y][my_row_x] == TR_ROOM_ROAD_CROSSR){frameX = 0;frameY = 4;}
		...
		if(room[my_row_y][my_row_x] >= TR_ROOM_HUT1 && room[my_row_y][my_row_x] <= TR_ROOM_HUT6)
		{
			if(room[my_row_y][my_row_x] == TR_ROOM_HUT1){frameX = 1;frameY = 4;} 
			if(room[my_row_y][my_row_x] == TR_ROOM_HUT2){frameX = 2;frameY = 4;}   
			if(room[my_row_y][my_row_x] == TR_ROOM_HUT3){frameX = 3;frameY = 4;}
			if(room[my_row_y][my_row_x] == TR_ROOM_HUT4){frameX = 4;frameY = 4;}
		...
		if(room[my_row_y][my_row_x] >= TR_ROOM_ROCK)
		{
			if(room[my_row_y][my_row_x] == TR_ROOM_ROCK){frameX = 7;frameY = 4;} 
		...
		if(room[my_row_y][my_row_x] == TR_ROOM_TREE)
		{
			if(room[my_row_y][my_row_x] == TR_ROOM_TREE){frameX = 7;frameY = 2;} 
		...
		if(room[my_row_y][my_row_x] == TR_ROOM_EMPTY)      
		{ 
...
{ 
	...
	if(room[testY][testX] == TR_ROOM_FULL)
		count++;
