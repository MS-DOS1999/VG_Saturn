...
void VG_Help_Story_Siu(int scene, int index)
{
	int rnd = 0;
	char punch;
	char punch2;
...
void VG_Init_Story_Siu(int index, int condition, int scene)
{
		...
		if(condition == VG_STORY_SCENE_LAST) 
		{
			...
			{   
				...
				if(VG_IS_AN_ALTER_P1 == false)
					sprintf( new_counter[1].word, "WITH SANG DEFEATED THE CITY IS SAFE.."); 
				else
					sprintf( new_counter[1].word, "WITH SANG DEFEATED THE CITY IS SAFE.."); 
			}
			if(index == 1) 
			{  
				...
				if(VG_IS_AN_ALTER_P1 == false)
					sprintf( new_counter[1].word, "BUT SIU WILL ALWAYS REMAIN VIGILANT!"); 
				else
					sprintf( new_counter[1].word, "DAE-SU CAN SPEND MORE TIME WITH HIS PIGEON!"); 
			}
			if(index == 2)   
			{  
				VG_Scene_char = 1;
			...
			if(VG_ARCADE_TYPE == VG_ARCADE_STANDARD && VG_DUELLISTS_UNLOCKED == 0)
			{
				...
				if(index == 4) 
				{
					if(VG_CHARACTER_LOCKS[VG_SIU] == 1)
						VG_Scene_char = 102; // Cause ending quit...
					else
					{  
						VG_CHARACTER_LOCKS[VG_SIU] = 1;
						VG_Scene_char = 9; // Sets the bg frame...
						VG_Scene_emotion = VG_EMOTION_HAPPY;
						sprintf( new_counter[1].word, "(DAE-SU IS NOW PLAYABLE!)"); 
						vg_pause_sfx = true;  
					}
				}
				if(index == 5)  
