...
void Run_Moves_REESE(int player, int f_rate)
{ 
		...
		{  
			...
			if (sprite[index].jumping == false)
			{
				...
				if (VG_reload_gun == false)
					if (sprite[index].special != 3 && sprite[index].special != 4) // Pistols...  && sprite[index].special != 5
						if (player == 1)
						{
							if (VG_1_special1_sfx == 0)
								VG_1_special1_sfx = 2;
	...
	if(sprite[index].special == 5)
	{
		...
		{ 
			...
			{ 
				if(sprite[index].movement == LEFT)
					New_FX(points[point_index].x-0.05f, points[point_index].y-0.05f,  points[point_index].x-0.05f, points[point_index].y-0.05f, bullet_anim, LOOP, 0.1f, 0.1f, 0.1f, 0.1f, 1, GFX, 1.0f, 0, index, FX_VG_ORB, index, 12, 0, 0, LEFT); 
				else
					New_FX(points[point_index].x-0.05f, points[point_index].y-0.05f,  points[point_index].x-0.05f, points[point_index].y-0.05f, bullet_anim, LOOP, 0.1f, 0.1f, 0.1f, 0.1f, 1, GFX, 1.0f, 0, index, FX_VG_ORB, index, 12, 0, 0, RIGHT); 
	...
	if(sprite[index].status == VG_SPECIAL1_FONT)       
	{
		...
		if(sprite[index].cur == 1)  
		{
			...
			if(sprite[index].movement == RIGHT)   
			{
				...
				points[point_index].y = sprite[index].y + hit_y;
				points[point_index+2].x = sprite[index].x + 0.30f;          
				points[point_index+2].y = sprite[index].y + hit_y;
			}  
			if(sprite[index].movement == LEFT)
			{
		...
		{     
			...
			{  
				...
				{  
					if(sprite[index].movement == RIGHT)
						New_FX(sprite[index].x+(sprite[index].width*0.9f), sprite[index].y, sprite[index].x, sprite[index].y, 30, LOOP, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 2, GFX, 1.0f, 0, -1, FX_ARM, index, sprite[index].id, 0, 0, index); 
					if(sprite[index].movement == LEFT)
						New_FX(sprite[index].x-(sprite[index].width*0.9f), sprite[index].y, sprite[index].x, sprite[index].y, 31, LOOP, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 2, GFX, 1.0f, 0, -1, FX_ARM, index, sprite[index].id, 0, 0, index); 
				}
				if(sprite[index].special == 2)
				{
					if(sprite[index].movement == RIGHT)
						New_FX(sprite[index].x+(sprite[index].width*0.9f), sprite[index].y-(sprite[index].height*0.225f), sprite[index].x, sprite[index].y, 30, LOOP, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 2, GFX, 1.0f, 0, -1, FX_ARM, index, sprite[index].id, 0, 0, index); 
					if(sprite[index].movement == LEFT)
						New_FX(sprite[index].x-(sprite[index].width*0.9f), sprite[index].y-(sprite[index].height*0.225f), sprite[index].x, sprite[index].y, 31, LOOP, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 2, GFX, 1.0f, 0, -1, FX_ARM, index, sprite[index].id, 0, 0, index); 
				}
				if(player == 1)
			...
			if(sprite[index].movement == RIGHT)   
			{
				points[point_index].x = sprite[index].x + 0.76f;          
				points[point_index].y = sprite[index].y + hit_y;
				points[point_index+2].x = sprite[index].x + 0.30f;          
				points[point_index+2].y = sprite[index].y + hit_y;
			}  
	...
	if(sprite[index].status == VG_LOW_KICK_FONT)     
	{
		...
		{    
			...
			if(sprite[index].movement == RIGHT)    
			{
				...
				points[point_index+2].x = sprite[index].x + 0.24f;             
				points[point_index+2].y = sprite[index].y + 0.12f;
			} 
			if(sprite[index].movement == LEFT)
			{
