...
void VG_Help_Story_Sang(int scene, int index)
{
	int rnd = 0;
	char punch;
	char punch2;
...
void VG_Init_Story_Sang(int index, int condition, int scene)
{
		...
		if(condition == VG_STORY_SCENE_LAST) 
		{
			...
			{   
				...
				if(VG_IS_AN_ALTER_P1 == false)
					sprintf( new_counter[1].word, "SANG CANNOT BE STOPPED NOW!"); 
				else
					sprintf( new_counter[1].word, "SANG CANNOT BE STOPPED NOW!"); 
			}
			if(index == 1) 
			{  
				...
				if(VG_IS_AN_ALTER_P1 == false)
					sprintf( new_counter[1].word, "HE RULES THE CITY WITH AN IRON FIST!"); 
				else
					sprintf( new_counter[1].word, "HE RULES THE CITY WITH AN IRON FIST!"); 
			}
			if(index == 2)   
