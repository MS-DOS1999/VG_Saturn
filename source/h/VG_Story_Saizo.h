...
void VG_Help_Story_Saizo(int scene, int index)
{
	int rnd = 0;
	char punch;
	char punch2;
...
void VG_Init_Story_Saizo(int index, int condition, int scene)
{
	...
	{  
		...
		if(condition == VG_STORY_SCENE_LAST) 
		{
			...
			{  
				...
				if(VG_IS_AN_ALTER_P1 == false)
					sprintf( new_counter[1].word, "SAIZO MISSES HIS ANIMAL FRIENDS!"); 
				else
					sprintf( new_counter[1].word, "SAIZO MISSES HIS ANIMAL FRIENDS!"); 
			}
			if(index == 2)   
