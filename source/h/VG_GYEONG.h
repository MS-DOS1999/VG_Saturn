...
void Run_Moves_GYEONG(int player, int f_rate)
{
	...
	{ 
		...
		{  
			...
			if(sprite[index].special != 3 && sprite[index].special != 4) // Pistols...
			if(player == 1) 
			{
				if(VG_1_special1_sfx == 0)
				    VG_1_special1_sfx = 2;   
	...
	/*		if(temp_VG_DOWN_pressed == false && sprite[index].jumping == false)
			if(VG_other_hits >= 2)
			{
				// Need to add punch short here... 
				move_anim = 14;
				Set_Sprite(index, move_anim, ONCE, ANIM_SPRITE, ANIM_NOT_WALKING); // VG_KICK_LONG_FONT
				sprite[index].status = VG_KICK_POWER_FONT;
			}*/ 
	...
	//			points[point_index].active = 1;    
		...
		if(sprite[index].special != 4) // Low pistol...
			...
			{    
				...
				{
					if(sprite[index].movement == LEFT)
						New_FX(points[point_index].x-0.02f, points[point_index].y-0.05f,  points[point_index].x-0.02f, points[point_index].y-0.05f, 8, LOOP, 0.1f, 0.1f, 0.1f, 0.1f, 1, GFX, 1.0f, sprite[index].movement, VG_shadow_index, FX_VG_CASING, VG_shadow_index, 12, 0, 0, (5+rand()%3)); 
					else
						New_FX(points[point_index].x-0.08f, points[point_index].y-0.05f,  points[point_index].x-0.08f, points[point_index].y-0.05f, 8, LOOP, 0.1f, 0.1f, 0.1f, 0.1f, 1, GFX, 1.0f, sprite[index].movement, VG_shadow_index, FX_VG_CASING, VG_shadow_index, 12, 0, 0, (5+rand()%3)); 
				...
				{
					if(sprite[index].movement == LEFT)
						New_FX(points[point_index].x+0.01f, points[point_index].y-0.05f,  points[point_index].x+0.01f, points[point_index].y-0.05f, 8, LOOP, 0.1f, 0.1f, 0.1f, 0.1f, 1, GFX, 1.0f, sprite[index].movement, VG_shadow_index, FX_VG_CASING, VG_shadow_index, 12, 0, 0, (5+rand()%3)); 
					else
						New_FX(points[point_index].x-0.11f, points[point_index].y-0.05f,  points[point_index].x-0.11f, points[point_index].y-0.05f, 8, LOOP, 0.1f, 0.1f, 0.1f, 0.1f, 1, GFX, 1.0f, sprite[index].movement, VG_shadow_index, FX_VG_CASING, VG_shadow_index, 12, 0, 0, (5+rand()%3)); 
			...
			// Slide back for pistol firing... 
			temp_speed = ((0.00015f/VG_slowmof)*(float)f_rate);
	...
	// Torpedo punch...
	if(sprite[index].special != 5) // Uppercut...
	if(sprite[index].status == VG_SPECIAL1_FONT)        
	{
		...
		{
			if(sprite[index].cur == 1)          
				temp_speed = ((0.00070f/VG_slowmof)*(float)f_rate); 

			if(sprite[index].cur == 2)      
				temp_speed = ((0.00200f/VG_slowmof)*(float)f_rate); 
	...
	if(sprite[index].status == VG_KICK_KNEE_FONT)     
	{
		...
		{      
			...
			{ 
				...
				points[point_index].y = sprite[index].y + 0.12f;
				points[point_index+2].x = sprite[index].x + 0.22f;       
				points[point_index+2].y = sprite[index].y + 0.22f;
			}   
	...
	if(sprite[index].status == VG_KICK_POWER_FONT)       
	{
		if(points[point_index].active != 2)	     
			points[point_index].active = 0;    
	...
	if(sprite[index].status == VG_JUMPING_PUNCH_FONT)       
	{
		...
		{   
			...
			{ 
				...
				points[point_index].y = sprite[index].y + 0.20f;
				points[point_index+2].x = sprite[index].x + 0.32f;          
				points[point_index+2].y = sprite[index].y + 0.26f;
			}  
			if(sprite[index].movement == LEFT)
