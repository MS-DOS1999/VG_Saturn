...
void Run_Moves_YOHAN(int player, int f_rate)
{
	...
	{ 
		...
		{  
 			...
 			if(sprite[index].special == 2)   
			{
 			...
 			if(sprite[index].special == 3) // Bomb drop...
			{
				if(player == 1) 
				{ 
					VG_1_Pfpsfx = 0;
			...
			if(sprite[index].special == 4)     
			{
 			...
 			if(sprite[index].special == 5) // Suicide...
			{
				move_anim = 68;
				Set_Sprite(index, move_anim, ONCE, ANIM_SPRITE, ANIM_NOT_WALKING); // VG_SPECIAL1_FONT
				sprite[index].status = VG_SPECIAL1_FONT;
	...
	if(sprite[index].status == VG_SPECIAL2_FONT)       
	{
		...
		{ 
			...
			if(sprite[index].special == 1) // Mid vial...      
			if(sprite[index].movement == RIGHT)     
			{  
			...
			if(sprite[index].special == 2) // Low vial...      
			if(sprite[index].movement == RIGHT)    
			{   
				points[point_index].x = sprite[index].x + 0.34f;           
				points[point_index].y = sprite[index].y + 0.18f;
			...
			{
				if(sprite[index].special == 1) 
					New_FX(points[point_index].x-0.05f, points[point_index].y-0.05f,  points[point_index].x-0.05f, points[point_index].y-0.05f, bullet_anim, LOOP, 0.1f, 0.1f, 0.1f, 0.1f, 1, GFX, 1.0f, 0, index, FX_VG_VIAL, -1, 12, 0, 0, -1); 
				if(sprite[index].special == 2)
					New_FX(points[point_index].x-0.05f, points[point_index].y-0.15f,  points[point_index].x-0.05f, points[point_index].y-0.15f, bullet_anim, LOOP, 0.1f, 0.1f, 0.1f, 0.1f, 1, GFX, 1.0f, 0, index, FX_VG_VIAL_LOW, -1, 12, 0, 0, -1); 
				VG_1_Vialsfx = 1;	  
			//	VG_SlowMO(500, 2.0f);
			}
			if(player == 2)  
	...
	if(sprite[index].special == 3)
	{
		...
		{   
			...
			if(VG_1_Pfpsfx == 0)  
			{
				New_FX(points[point_index].x-0.05f, points[point_index].y-0.05f,  points[point_index].x-0.05f, points[point_index].y-0.05f, 23, ONCE, 0.1f, 0.1f, 0.1f, 0.1f, 1, GFX, 1.0f, 0, index, FX_VG_BOMB, -1, 12, 0, 0, player); 
				VG_1_Pfpsfx = 1;	  
		...
		if(sprite[index].cur == 1)          
			temp_speed = ((0.00350f/VG_slowmof)*(float)f_rate); 

	//	if(sprite[index].cur == 2)          
	//		temp_speed = ((0.00002f/VG_slowmof)*(float)f_rate); 
	...
	if(sprite[index].status == VG_SPECIAL1_FONT)       
	{
		...
		{   
			...
			if((int)TimerGetTime() > VG_1_trail_timer)
			{
				...
				if(sprite[index].movement == RIGHT)
					New_FX(sprite[index].x, sprite[index].y, sprite[index].x, sprite[index].y, 74, PLAY, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 3, GFX, 1.0f, 0, -1, FX_STATIC, index, sprite[index].id, 0, 0, index);
				else
					New_FX(sprite[index].x, sprite[index].y, sprite[index].x, sprite[index].y, 74, PLAY, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 3, GFX, 1.0f, 0, -1, FX_FLIP, index, sprite[index].id, 0, 0, index);
			...
			{ 
				...
				points[point_index+2].y = sprite[index].y + 0.05f;
			} 
			if(sprite[index].movement == LEFT)
			{
	...
	{ 
		if(sprite[index].cur == 2)   
		{
			...
			if(sprite[index].sticky == false)
				New_FX(sprite[index].x + 0.15f, sprite[index].y + 0.35f, sprite[index].x + 0.15f, sprite[index].y + 0.35f, 24, LOOP, sprite[index].width/4,sprite[index].height/4, sprite[index].width/4,sprite[index].height/4, 2, GFX, 1.0f, 0, -1, FX_VG_STICKYS, index, 12, 9, 0, index); // FX_VG_ATTACH

			sprite[index].sticky = true;
	...
	if(sprite[index].status == VG_SECOND_STRIKE)     
	{
		...
		{   
			...
			if(sprite[index].movement == RIGHT) 
			{
				...
				points[point_index].y = sprite[index].y + 0.25f;
				points[point_index+2].x = sprite[index].x + 0.30f;             
				points[point_index+2].y = sprite[index].y + 0.25f;
			}  
			if(sprite[index].movement == LEFT)  
			{
	...
	if(sprite[index].status == VG_JUMPING_KICK_FONT)      
	{
		...
		{   
			...
			{ 
				...
				points[point_index].y = sprite[index].y + 0.14f;
				points[point_index+2].x = sprite[index].x + 0.20f;          
