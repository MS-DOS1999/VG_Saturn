...
void VG_Help_Story_Minso(int scene, int index)
{
	int rnd = 0;
	char punch;
	char punch2;
...
void VG_Init_Story_Minso(int index, int condition, int scene)
{
		...
		if(condition == VG_STORY_SCENE_LAST) 
		{
			...
			{    
				...
				if(VG_IS_AN_ALTER_P1 == false)
					sprintf( new_counter[1].word, "FINALLY MINSO CAN GET HER KIMCHI!"); 
				else
					sprintf( new_counter[1].word, "WITH SANG DEFEATED AND KYUNG NOW SAFE"); 
			}
			if(index == 1) 
			{  
				VG_Scene_char = 2; 
				...
				if(VG_IS_AN_ALTER_P1 == false)
					sprintf( new_counter[1].word, "SHE$S BEEN STARVING THE WHOLE TIME!"); 
				else
					sprintf( new_counter[1].word, "MIA GOES ON TO BECOME DISTRICT CHIEF!"); 
			}
			if(index == 2)   
			{    
				VG_Scene_char = 2;
			...
			if(VG_ARCADE_TYPE == VG_ARCADE_STANDARD && VG_DUELLISTS_UNLOCKED == 0)
			{
				...
				if(index == 4) 
				{
					if(VG_CHARACTER_LOCKS[VG_MINSO] == 1)
						VG_Scene_char = 102; // Cause ending quit...
					else
					{  
						VG_CHARACTER_LOCKS[VG_MINSO] = 1;
						VG_Scene_char = 9; // Sets the bg frame...
						VG_Scene_emotion = VG_EMOTION_HAPPY;
						sprintf( new_counter[1].word, "(MIA IS NOW PLAYABLE!)"); 
						vg_pause_sfx = true;  
					}
				}
				if(index == 5) 
