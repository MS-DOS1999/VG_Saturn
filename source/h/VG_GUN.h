...
void Run_Moves_GUN(int player, int f_rate)
{
	...
	{ 
		...
		{   
			...
			if(sprite[index].special == 1) // Flying Uppercut...
			{ 
				move_anim = 9;
				Set_Sprite(index, move_anim, ONCE, ANIM_SPRITE, ANIM_NOT_WALKING); // VG_SPECIAL1_FONT
				sprite[index].status = VG_SPECIAL1_FONT;
			}
			if(sprite[index].special == 2) // Cannonball...
			{
				move_anim = 27;
				Set_Sprite(index, move_anim, ONCE, ANIM_SPRITE, ANIM_NOT_WALKING); // VG_SPECIAL1_FONT
				sprite[index].status = VG_SPECIAL1_FONT;
			}
			if(sprite[index].special == 3)  // Clothesline...
			{
			...
			if(sprite[index].special == 4)  // Whiplash...
			{
	...
	{ 
		...
		if(sprite[index].cur >= 1 && sprite[index].cur < 12) 
		{ 
			...
			if(sprite[index].movement == RIGHT)   
			{
				...
				points[point_index+2].x = sprite[index].x + 0.00f;          
			...
			if(sprite[index].movement == LEFT)
			{
				...
				points[point_index].y = sprite[index].y + 0.32f;
				points[point_index+2].x = sprite[index].x + 0.40f;          
				points[point_index+2].y = sprite[index].y + 0.32f;
			}  
		}
	...
	if(sprite[index].status == VG_SPECIAL2_FONT)  
	{
		...
		{ 
			...
			{  
				if(sprite[index].movement == RIGHT)
					New_FX(sprite[index].x+(sprite[index].width/2), sprite[index].y + (sprite[index].height*0.5f), sprite[index].x, sprite[index].y, 69, LOOP, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 2, GFX, 1.0f, 0, -1, FX_ARM, index, sprite[index].id, 0, 0, index); 
				if(sprite[index].movement == LEFT)
					New_FX(sprite[index].x-(sprite[index].width/2), sprite[index].y + (sprite[index].height*0.5f), sprite[index].x, sprite[index].y, 70, LOOP, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 2, GFX, 1.0f, 0, -1, FX_ARM, index, sprite[index].id, 0, 0, index); 
			...
			if(sprite[index].movement == RIGHT)  
			{
				points[point_index].x = sprite[index].x + 0.54f;          
				points[point_index].y = sprite[index].y + 0.5f;
				points[point_index+2].x = sprite[index].x + 0.30f;          
				points[point_index+2].y = sprite[index].y + 0.29f;
			}  
			if(sprite[index].movement == LEFT)
			{ 
				points[point_index].x = sprite[index].x - 0.14f;          
				points[point_index].y = sprite[index].y + 0.5f;
				points[point_index+2].x = sprite[index].x + 0.16f;          
				points[point_index+2].y = sprite[index].y + 0.29f;
			}  
	...
	if(sprite[index].special == 1)
	{
		...
		{   
			...
			if((int)TimerGetTime() > VG_1_trail_timer)
			{
				...
				if(sprite[index].movement == RIGHT)
					New_FX(sprite[index].x, sprite[index].y, sprite[index].x, sprite[index].y, 43, PLAY, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 3, GFX, 1.0f, 0, -1, FX_STATIC, index, sprite[index].id, 0, 0, index);
				else
					New_FX(sprite[index].x, sprite[index].y, sprite[index].x, sprite[index].y, 43, PLAY, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 3, GFX, 1.0f, 0, -1, FX_FLIP, index, sprite[index].id, 0, 0, index);
		...
		if(sprite[index].cur == 2)      
			temp_speed = ((0.00160f/VG_slowmof)*(float)f_rate); 
	...
	if(sprite[index].special == 2)
	{
		...
		{   
			...
			if((int)TimerGetTime() > VG_1_trail_timer)  
			{
				...
				if(sprite[index].movement == RIGHT)
					New_FX(sprite[index].x, sprite[index].y, sprite[index].x, sprite[index].y, 44, PLAY, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 3, GFX, 1.0f, 0, -1, FX_STATIC, index, sprite[index].id, 0, 0, index);
				else
					New_FX(sprite[index].x, sprite[index].y, sprite[index].x, sprite[index].y, 44, PLAY, sprite[index].width,sprite[index].height, sprite[index].width,sprite[index].height, 3, GFX, 1.0f, 0, -1, FX_FLIP, index, sprite[index].id, 0, 0, index);
			...
			{ 
				...
				points[point_index].y = sprite[index].y + 0.17f;
				points[point_index+2].x = sprite[index].x + 0.18f;          
				points[point_index+2].y = sprite[index].y + 0.17f;
			}  
		...
		if(sprite[index].cur == 1)          
			temp_speed = ((0.00175f/VG_slowmof)*(float)f_rate); 

		if(sprite[index].cur == 2)       
			temp_speed = ((0.00325f/VG_slowmof)*(float)f_rate); 
	...
	if(sprite[index].status == VG_PUNCH_MEDIUM_FONT)       
	{
		...
		{   
			...
			if(sprite[index].movement == RIGHT)   
			{
				points[point_index].x = sprite[index].x + 0.59f;          
				points[point_index].y = sprite[index].y + 0.3f;
			...
			if(sprite[index].movement == LEFT)   
			{
				points[point_index].x = sprite[index].x - 0.19f;          
				points[point_index].y = sprite[index].y + 0.3f;
	...
	if(sprite[index].status == VG_THIRD_STRIKE)     
	{
		...
		if(sprite[index].cur == 0 || sprite[index].cur == 2)     
		{   
			...
			if(sprite[index].movement == RIGHT)      
			{  
				points[point_index].x = sprite[index].x + 0.57f;            
				points[point_index].y = sprite[index].y + 0.18f;
				points[point_index+2].x = sprite[index].x + 0.34f;              
			...
			if(sprite[index].movement == LEFT)
			{
				points[point_index].x = sprite[index].x - 0.17f;        
				points[point_index].y = sprite[index].y + 0.18f;  
				points[point_index+2].x = sprite[index].x + 0.06f;       
				points[point_index+2].y = sprite[index].y + 0.22f;
			}
...
/*	if(sprite[index].status == VG_UPPERCUT_FONT)     
	...
	if(sprite[index].status == VG_UPPERCUT_FONT)       
	{
		...
		{   
			...
			if(sprite[index].movement == RIGHT)   
			{
				...
				points[point_index].y = sprite[index].y + 0.38f;
				points[point_index+2].x = sprite[index].x + 0.46f;             
				points[point_index+2].y = sprite[index].y + 0.3f;
			}  
			if(sprite[index].movement == LEFT)   
			{
	...
	if(sprite[index].status == VG_JUMPING_KICK_FONT)      
	{
		...
		{   
			...
			if(sprite[index].movement == RIGHT)   
			{ 
				points[point_index].x = sprite[index].x + 0.43f;          
				points[point_index].y = sprite[index].y + 0.31f;
				points[point_index+2].x = sprite[index].x + 0.30f;          
				points[point_index+2].y = sprite[index].y + 0.31f;
			}  
			if(sprite[index].movement == LEFT)
			{
