...
void Run_Moves_TIA(int player, int f_rate)
{
	...
	{  
		...
		{  
			...
			if(sprite[index].jumping == false) //  && sprite[index].on_ladder == false
			{
				// Length of the move...   
				if(player == 1)    
				{
	...
	if(sprite[index].status == VG_SPECIAL1_FONT)       
	{
		...
		if (sprite[index].cur == 1 || sprite[index].cur == 2)
		{
			sprite[index].invulnerable = true;
			...
			if (player == 1)
				if ((int)TimerGetTime() > VG_1_trail_timer)
				{
					...
					if (sprite[index].movement == RIGHT)
						New_FX(sprite[index].x, sprite[index].y, sprite[index].x, sprite[index].y, 77, PLAY, sprite[index].width, sprite[index].height, sprite[index].width, sprite[index].height, 3, GFX, 1.0f, 0, -1, FX_STATIC, index, sprite[index].id, 0, 0, index);
					else
						New_FX(sprite[index].x, sprite[index].y, sprite[index].x, sprite[index].y, 77, PLAY, sprite[index].width, sprite[index].height, sprite[index].width, sprite[index].height, 3, GFX, 1.0f, 0, -1, FX_FLIP, index, sprite[index].id, 0, 0, index);
