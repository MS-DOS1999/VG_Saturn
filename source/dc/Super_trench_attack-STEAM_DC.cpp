...
void AlertSmooth(int reduce, unsigned int tex, float U, float V, float U1, float V1)
{
	...
	if (reduce == 0)
	{
		...
		while(Counter <= 15)
		{
			float ratioX = float(Counter) * (10.666*2);
			float ratioY = float (Counter) * (8.0*2);
			glBindTexture(GL_TEXTURE_2D, tex);
			glMatrixMode(GL_PROJECTION);
			glLoadIdentity();
			glOrtho(0.0, 640.0, 0.0, 480.0, -1.0, 1.0);   // setup a 10x10x2 viewing world
			...
			glTexCoord2f(U, V1); glVertex3f(320.0 - ratioX, 240.0 - ratioY, 0.0);
			glTexCoord2f(U, V); glVertex3f(320.0 - ratioX, 240.0 + ratioY, 0.0);
			glTexCoord2f(U1, V); glVertex3f(320.0 + ratioX, 240.0 + ratioY, 0.0);
			glTexCoord2f(U1, V1); glVertex3f(320.0 + ratioX, 240.0 - ratioY, 0.0);
			glEnd();
	...
	else if(reduce == 1)
	{
		...
		glBegin(GL_QUADS);
		glNormal3f(0.0, 0.0, 1.0);
		glTexCoord2f(U, V1); glVertex3f(0.0, 0.0, 0.0);
		glTexCoord2f(U, V); glVertex3f(0.0, 480.0, 0.0);
		glTexCoord2f(U1, V); glVertex3f(640.0, 480.0, 0.0);
		glTexCoord2f(U1, V1); glVertex3f(640.0, 0.0, 0.0);
		glEnd();
	...
	else
	{
		...
		while(Counter <= 15)
		{
			...
			glBegin(GL_QUADS);
			glNormal3f(0.0, 0.0, 1.0);
			glTexCoord2f(U, V1); glVertex3f(0.0 + ratioX, 0.0 + ratioY, 0.0);
			glTexCoord2f(U, V); glVertex3f(0.0 + ratioX, 480.0 - ratioY, 0.0);
			glTexCoord2f(U1, V); glVertex3f(640.0 - ratioX, 480.0 - ratioY, 0.0);
			glTexCoord2f(U1, V1); glVertex3f(640.0 - ratioX, 0.0 + ratioY, 0.0);
			glEnd();
...
int main(int argc, char **argv)
{
	...
	*/
    ...
    config.initial_pt_capacity = 1024 * 3;
...
//	MAX_vertical   = GetSystemMetrics(SM_CYSCREEN);
    ...
    {
    	a_texture[xyz].anim_frames = NULL;
    	a_texture[xyz].delay = NULL;
    	a_texture[xyz].anims = NULL;
    }
 ...
 //SPLASH SCREEN PIXEL
	...
	//Load_KMG_inde("VMUALERT.png.tex", &VMU_ALERT);

	Image stexture_vmualert;

	if (!ImageLoad("cd/data/VMUALERT.tex", &stexture_vmualert)) {
		printf("Can't Load Texture %s\n", "cd/data/VMUALERT.tex");
		return 0;
	}

	printf("Load Texture %s\n", "cd/data/VMUALERT.tex");

	unsigned int texID_vmu;

	glGenTextures(1, &texID_vmu);

	glBindTexture(GL_TEXTURE_2D, texID_vmu);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

	glCompressedTexImage2DARB(GL_TEXTURE_2D, 0, stexture_vmualert.internalFormat, stexture_vmualert.sizeX, stexture_vmualert.sizeY, 0, stexture_vmualert.dataSize, stexture_vmualert.data);

	free(stexture_vmualert.data);
	//Load_KMG_inde("VMUSAVELOAD.png.tex", &VMU_ALERT);
	
	maple_device_t *vmu;
	if(!(vmu = maple_enum_dev(0, 1))) {
	  printf("NO VMU DETECTED\n");
        AlertSmooth(0, texID_vmu, 0.0, 0.0, 0.625, 0.46875);
        while(!(vmu = maple_enum_dev(0, 1)))
        {
        	//printf("Enter Loop\n");
        	UpdateControllersStatus();
        	if(ButtonPushed(0, CONT_A))
        	{
        		continueWithoutSaving = true;
        		break;
        	}
        	AlertSmooth(1, texID_vmu, 0.0, 0.0, 0.625, 0.46875);
        }
        AlertSmooth(2, texID_vmu, 0.0, 0.0, 0.625, 0.46875);
    	}
    	else if(!vmu->valid || !(vmu->info.functions & MAPLE_FUNC_MEMCARD))
    	{
    		printf("NO VMU DETECTED 2\n");
    		AlertSmooth(0, texID_vmu, 0.0, 0.0, 0.625, 0.46875);
	      while(!vmu->valid || !(vmu->info.functions & MAPLE_FUNC_MEMCARD))
	      {
	        	UpdateControllersStatus();
	        	if(ButtonPushed(0, CONT_A))
	        	{
    	...
    	if(continueWithoutSaving == false)
    	{
    		...
    		uint8* dataGet = (uint8*)malloc(500);
    		int res = read_from_vmu(dataGet);
    		memset(dataGet, 0, 500);
    		dataGet[0] = 'n';
    		dataGet[1] = 'o';
    		dataGet[2] = '!';
    		dataGet[3] = '\0';
    		...
    		if (res == -1)
    		{
    			...
    			res = save_to_vmu(dataGet, 500);
    			printf("SAVE RESULT FIRST %d\n", res);
    			if(res != 0)
    			{
    				printf("NO ENOUGH MEM\n");
    				AlertSmooth(0, texID_vmu, 0.0, 0.46975, 0.625,  0.46975+0.46875);
    				while(res != 0)
    				{
    					UpdateControllersStatus();
			        	if(ButtonPushed(0, CONT_A))
			        	...
			        	AlertSmooth(1, texID_vmu, 0.0, 0.46975, 0.625, 0.46975+0.46875);
			        	res = save_to_vmu(dataGet, 500);
			        	printf("SAVE RESULT SECOND %d\n", res);
			        	
    				}
    				AlertSmooth(2, texID_vmu, 0.0, 0.46975, 0.625,  0.46975+0.46875);
    			}
    		}
    		free(dataGet);
    	}

    	glDeleteTextures(1, &texID_vmu);
	...
	Image stexture_pixelsplash;

	if (!ImageLoad("cd/data/Josh.tex", &stexture_pixelsplash)) {
		printf("Can't Load Texture %s\n", "cd/data/Josh.tex");
		return 0;
	}
	...
	glCompressedTexImage2DARB(GL_TEXTURE_2D, 0, stexture_pixelsplash.internalFormat, stexture_pixelsplash.sizeX, stexture_pixelsplash.sizeY, 0, stexture_pixelsplash.dataSize, stexture_pixelsplash.data);

	free(stexture_pixelsplash.data);

	//int time_splash = CTimerGet();

	//TEST
	int startuptime = TimerGetTime();
	int timePassedSplash = 0;
	while (timePassedSplash < 2000)
	{
		...
		timePassedSplash = TimerGetTime() - startuptime;
		//printf("timePassed = %d\n", timePassedSplash);
	}

	glDeleteTextures(1, &texID_splash);

	printf("PASS CREATE WINDOWS !\n");
    ...
    SoundEffectPtr vg_lose_snd(OpenSoundEffect(device, "Data/SFX/VG_LOSE.wav", SINGLE));
		...
		OutputStreamPtr emy1_snd(OpenSound(device, "BG_ALLEYWAY.adx", STREAM_MUSIC));        // Title screen 
		OutputStreamPtr emy2_snd(OpenSound(device, "BG_TRAIN.adx", STREAM_MUSIC));        // Title screen 
		OutputStreamPtr emy3_snd(OpenSound(device, "BG_TRAFFIC.adx", STREAM_MUSIC));        // Title screen 
		OutputStreamPtr emy4_snd(OpenSound(device, "BG_PARK.adx", STREAM_MUSIC));        // Title screen 
		OutputStreamPtr emy5_snd(OpenSound(device, "BG_BIKESHOP.adx", STREAM_MUSIC));        // Title screen 
		OutputStreamPtr emy6_snd(OpenSound(device, "BG_NIGHTCLUB.adx", STREAM_MUSIC));        // Title screen
		OutputStreamPtr emy7_snd(OpenSound(device, "BG_PRISON.adx", STREAM_MUSIC));        // Title screen 
		OutputStreamPtr emy8_snd(OpenSound(device, "BG_HIGHWAY.adx", STREAM_MUSIC));        // Title screen 
		OutputStreamPtr emy9_snd(OpenSound(device, "BG_COURT.adx", STREAM_MUSIC));        // Title screen 
		OutputStreamPtr emy10_snd(OpenSound(device, "BG_DOJO.adx", STREAM_MUSIC));        // Title screen 
		OutputStreamPtr emy11_snd(OpenSound(device, "BG_AIRPORT.adx", STREAM_MUSIC));  
		OutputStreamPtr emy12_snd(OpenSound(device, "BG_ROOFTOPS.adx", STREAM_MUSIC));  
		OutputStreamPtr emy13_snd(OpenSound(device, "BG_CAFE.adx", STREAM_MUSIC));  
		OutputStreamPtr emy14_snd(OpenSound(device, "BG_RING.adx", STREAM_MUSIC)); 

		OutputStreamPtr jet1_snd(OpenSound(device, "VG_VS.adx", STREAM_MUSIC));      // Old Radio system 
		OutputStreamPtr jet2_snd(OpenSound(device, "VG_GAMEOVER.adx", STREAM_MUSIC));      // Old Radio system 
		OutputStreamPtr jet3_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));      // Old Radio system 
		OutputStreamPtr jet4_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));      // Old Radio system 

		OutputStreamPtr siege1_snd(OpenSound(device, "CS_DANGER1.adx", STREAM_MUSIC));      // Old Radio system  
        OutputStreamPtr siege2_snd(OpenSound(device, "CS_DANGER2.adx", STREAM_MUSIC));      // Old Radio system
        OutputStreamPtr siege3_snd(OpenSound(device, "CS_VICTORY.adx", STREAM_MUSIC));      // Old Radio system
        OutputStreamPtr siege4_snd(OpenSound(device, "CS_ENDING.adx", STREAM_MUSIC));      // Old Radio system
        OutputStreamPtr siege5_snd(OpenSound(device, "CS_BOSSIDLE.adx", STREAM_MUSIC));      // Old Radio system
        OutputStreamPtr siege6_snd(OpenSound(device, "CS_IDLE.adx", STREAM_MUSIC));      // Old Radio system
    ...
    OutputStreamPtr map_grass_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));        // Credits screen
    OutputStreamPtr map_swamp_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));        // Credits screen
    OutputStreamPtr map_forest_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));        // Credits screen
    OutputStreamPtr map_farm_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));        // Credits screen
    OutputStreamPtr map_snow_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));        // Credits screen
    OutputStreamPtr map_desert_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));        // Credits screen
	OutputStreamPtr map_trench_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));        // Credits screen

    OutputStreamPtr lnd_snd(OpenSound(device, "CS_ENDING.adx", STREAM_MUSIC));       // File select screen
    OutputStreamPtr theatre_snd(OpenSound(device, "CS_INTRO.adx", STREAM_MUSIC));      // Old Radio system     
    OutputStreamPtr credits1_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));      // Old Radio system 
    OutputStreamPtr credits2_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));      // Old Radio system 
    OutputStreamPtr credits3_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));      // Old Radio system 
    ...
    OutputStreamPtr crd_snd(OpenSound(device, "VG_SELECT.adx", STREAM_MUSIC));
            
    OutputStreamPtr army_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));  
    ...
    OutputStreamPtr fight_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));
            
    OutputStreamPtr sad_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));    
            
    OutputStreamPtr intro_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC)); 
            
    OutputStreamPtr mini_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC)); 
            
    OutputStreamPtr deathscreen_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC)); 
                        
    OutputStreamPtr urgency_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC)); 
                
    OutputStreamPtr fanfare_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC)); 
                
    OutputStreamPtr fanfare2_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC)); 
    ...
    OutputStreamPtr dgnb_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));  
            
    OutputStreamPtr final_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));  
            
    OutputStreamPtr dangers_snd(OpenSound(device, "BLANK.adx", STREAM_MUSIC));  
    ...
    OutputStreamPtr vg_amb_prison_snd(OpenSound(device, "VG_AMB_PRISON.adx", STREAM_MUSIC)); 
            
    OutputStreamPtr vg_amb_traffic_snd(OpenSound(device, "VG_AMB_TRAFFIC.adx", STREAM_MUSIC)); 

	OutputStreamPtr vg_amb_subway_snd(OpenSound(device, "VG_AMB_SUBWAY.adx", STREAM_MUSIC)); 

    OutputStreamPtr vg_amb_van_snd(OpenSound(device, "VG_VAN.adx", STREAM_MUSIC));

    OutputStreamPtr vg_amb_duel_snd(OpenSound(device, "VG_AMB_DUEL.adx", STREAM_MUSIC));
...
{   
    ...
    {               
        ...
        {      
			...
			if( Fx[end_id].anim >= 0 && Fx[end_id].anim < 256)
			if( Fx[end_id].cur >= 0 && Fx[end_id].cur < 256)    
            	Fx[end_id].frame = a_texture_get_anims_secondlayer(Fx[end_id].fx_id, Fx[end_id].anim, Fx[end_id].cur);
#endif
